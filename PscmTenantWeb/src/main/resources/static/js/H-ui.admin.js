...
function Hui_admin_tab_tree(obj){
	var bStop = false,
		bStopIndex = 0,
		href = $(obj).attr('dataUrl'),
		title = $(obj).attr("name"),
		id = $(obj).attr("id"),
		topWindow = $(window.parent.document),
		show_navLi = topWindow.find("#min_title_list li"),
	...
	show_navLi.each(function() {
		if($(this).find('span').attr("idKey")==id){
			bStop=true;
			bStopIndex=show_navLi.index($(this));
			return false;
		}
...
function creatIframe(href,titleName,id){
	...
	show_nav.append('<li class="active"><span data-href="'+href+'" idKey="' + id + '">'+titleName+'</span><em></em></li>');
	...
	iframeBox.hide();
	iframe_box.append('<div class="show_iframe"><div class="loading"></div><iframe frameborder="0" id="mainframe_' + id + '" name="mainframe_' + id + '" src='+href+'></iframe></div>');
